...
4. Click **Connect**. The app calls `capabilities.integrations.connect("google-composio", { scopes })` with only the selected scopes, opening Google's OAuth consent popup. The app stays open; when the popup resolves, the new account appears in the connected-accounts list with its granted capabilities shown as badges.
...
2. **For an email, scheduled, or background turn, select explicitly.** Use `capabilities.integrations.list("google-composio")` to enumerate connected accounts and their granted `scopes`. Choose a connection whose `scopes` include the scope required by the requested tool:
